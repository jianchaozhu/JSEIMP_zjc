...
-(void)setupUI{
    ...
    for (int i = 0; i < _titleMArray.count; i++) {
        ...
        _textView = [self setupTextView];
        _textView.frame = CGRectMake(210, 302 + 50 * i, UIScreenW -212, 46);
        _textView.text = @"”不满意事例“的说明是xxxx";
        _textView.editable = NO;
        _textView.textColor = [UIColor darkTextColor];
        
//        _label15 = [self setupLabelWithText:@"" TextColor:[UIColor darkTextColor] Font:[UIFont systemFontOfSize:16]];
//        _label15.frame = CGRectMake(4, 4, _textView.frame.size.width - 8, 16);
    }
    ...
    [_view mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(_label12.mas_bottom).offset(16);
    }];
}
