...
- (void)layoutSubviews{
    ...
    for (int i = 0; i < numberOfCells; i++) {
        ...
        _rootScrollViewcell = self.displayingPageViews[@(i)];
        if ([self isInScreen:cellFrame]) {
            if (_rootScrollViewcell == nil) {
                _rootScrollViewcell = [self.rootScrollViewDateSource rootScrollView:self AtIndex:i];
                _rootScrollViewcell.frame = CGRectMake(cellFrame.origin.x, cellFrame.origin.y, cellFrame.size.width, cellFrame.size.height);
                
                [self addSubview:_rootScrollViewcell];
                
                
                // 存放到字典中
                self.displayingPageViews[@(i)] = _rootScrollViewcell;
            }
        }else{
            if (_rootScrollViewcell) {
                // 从scrollView和字典中移除
                [_rootScrollViewcell removeFromSuperview];
                [self.displayingPageViews removeObjectForKey:@(i)];
                // 存放进缓存池
                [self.reusePageViews addObject:_rootScrollViewcell];
            }
        }
    }
}
