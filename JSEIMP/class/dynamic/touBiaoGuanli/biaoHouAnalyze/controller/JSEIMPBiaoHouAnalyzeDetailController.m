//
//  JSEIMPBiaoHouAnalyzeDetailController.m
//  JSEIMP
//
...
@interface JSEIMPBiaoHouAnalyzeDetailController ()

@end

@implementation JSEIMPBiaoHouAnalyzeDetailController{
    ...
    UILabel *_kaiBiaoMoneyLabel;
    
    UILabel *_isZhongBiaoLabel;

    UILabel *_analyzeLabel;
...
-(void)setupUI{
    ...
    _label7 = [self setupLabelWithText:@"开标日期" TextColor:[UIColor darkTextColor] Font:[UIFont systemFontOfSize:20]];
    _label8 = [self setupLabelWithText:@"开标总价(万元)" TextColor:[UIColor darkTextColor] Font:[UIFont systemFontOfSize:20]];
    _label9 = [self setupLabelWithText:@"是否中标" TextColor:[UIColor darkTextColor] Font:[UIFont systemFontOfSize:20]];
    _label10 = [self setupLabelWithText:@"标后分析" TextColor:[UIColor darkTextColor] Font:[UIFont systemFontOfSize:20]];
    ...
    _startDateLabel = [self setupLabelWithText:@"2015-08-08" TextColor:[UIColor darkGrayColor] Font:[UIFont systemFontOfSize:16]];
    _startDateLabel.textAlignment = NSTextAlignmentRight;
    
    _kaiBiaoMoneyLabel = [self setupLabelWithText:@"80万元" TextColor:[UIColor darkGrayColor] Font:[UIFont systemFontOfSize:16]];
    _kaiBiaoMoneyLabel.textAlignment = NSTextAlignmentRight;
    
    _isZhongBiaoLabel = [self setupLabelWithText:@"是" TextColor:[UIColor darkGrayColor] Font:[UIFont systemFontOfSize:16]];
    _isZhongBiaoLabel.textAlignment = NSTextAlignmentRight;
    
    _analyzeLabel = [self setupLabelWithText:@"1.页面，已发布的公告列表内容排布正常,发布人员等相关信息正确，点击新闻发布跳转到新闻发布页面。 2.内容有效性判断，必填字段不可为空，公告内容中填写特殊字符和html关键字内容提交，显示正确， 3.公告内容预览以及查看时和原格式保持一致。 4.页面按钮功能检测，如各条目的展开和收起，编辑框中上的各页面各按钮如字体选择和段落格式选择切换和基本功能正常，各条目总数统计正常，页码标签跳转正常" TextColor:[UIColor darkGrayColor] Font:[UIFont systemFontOfSize:16]];
    _analyzeLabel.numberOfLines = 0;
    ...
    [_kaiBiaoMoneyLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.centerY.mas_equalTo(_label8.mas_centerY);
        make.left.mas_equalTo(_label8.mas_right).offset(16);
        make.right.mas_equalTo(_startDateLabel.mas_right);
    ...
    [_isZhongBiaoLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(_startDateLabel.mas_left);
        make.right.mas_equalTo(_startDateLabel.mas_right);
    ...
    [_analyzeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_view mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(_analyzeLabel.mas_bottom).offset(16);
